...
if($pre_text) {
  print '<p class="rapid_contact intro_text ' . $mod_class_suffix . '">'.$pre_text.'</p>' . "\n";
}
...
if ($enable_anti_spam) {
  if ($anti_spam_position == 0) {
    print '<div>' . $emptycell.'<input class="rapid_contact inputbox antispam ' . $mod_class_suffix . '" type="text" name="rp_anti_spam_answer" size="' . $emailWidth . '" value="'.$CORRECT_ANTISPAM_ANSWER.'" placeholder="'.$myAntiSpamQuestion.'"/></div>' . "\n";
  }
}
// print first name
print '<div>' . '<input class="rapid_contact inputbox ' . $mod_class_suffix . '" type="text" name="rp_first_name" size="' . $firstNameWidth . '" value="'.$CORRECT_FIRST_NAME.'" placeholder="'.$myFirstNameLabel.'"/>';
// print last name
print '<input class="rapid_contact inputbox ' . $mod_class_suffix . '" type="text" name="rp_last_name" size="' . $lastNameWidth . '" value="'.$CORRECT_LAST_NAME.'" placeholder="'.$myLastNameLabel.'"/></div>';
// print phone number
print '<div>' . '<input class="rapid_contact inputbox ' . $mod_class_suffix . '" type="text" name="rp_phone_number" size="' . $phoneNumberWidth . '" value="'.$CORRECT_PHONE_NUMBER.'" placeholder="'.$myPhoneNumberLabel.'"/>';
// print email input 
print '<input class="rapid_contact inputbox ' . $mod_class_suffix . '" type="text" name="rp_email" size="' . $emailWidth . '" value="'.$CORRECT_EMAIL.'" placeholder="'.$myEmailLabel.'"/></div>';
// print message input
print '<div><textarea placeholder="'.$myMessageLabel.'" class="rapid_contact textarea ' . $mod_class_suffix . '" name="rp_message" cols="' . $messageWidth . '" rows="4" >'.$CORRECT_MESSAGE.'</textarea></div>';
print '<div><input class="rapid_contact button ' . $mod_class_suffix . '" type="submit" value="' . $buttonText . '" /></div><div class="clr"></div>';
//print anti-spam
if ($enable_anti_spam) {
